import numpy as np
from linear_regression_gradient_descent import lasso_regression_grad_desc, pred_and_r2, theta_to_function
...
def test_synthetic1_by_score() :
    ...
    X_test = X[800:]
    y_test = y[800:]
    theta = lasso_regression_grad_desc(X_train, y_train)
    reg = theta_to_function(theta)
    assert pred_and_r2(reg, X_train, y_train) == approx(1., .05)
    assert pred_and_r2(reg, X_test, y_test) == approx(1., .02)
...
def test_synthetic2_by_score() :
    ...
    theta = lasso_regression_grad_desc(X_train, y_train)
    reg = theta_to_function(theta)
    assert pred_and_r2(reg, X_train, y_train) == approx(1., .03)
    assert pred_and_r2(reg, X_test, y_test) == approx(1., .01)

def test_diabetes() :
    diabetes = load_diabetes()
    reg = theta_to_function(lasso_regression_grad_desc(diabetes.data[:400], diabetes.target[:400], alpha=.1))
...
def test_mpg() :
    ...
    reg = theta_to_function(lasso_regression_grad_desc(X_train, y_train))
