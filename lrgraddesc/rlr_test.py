import numpy as np
from linear_regression_gradient_descent import ridge_regression_grad_desc, pred_and_r2, theta_to_function
...
def test_synthetic1_by_theta() :
    ...
    X_test = X[800:]
    y_test = y[800:]
    theta = ridge_regression_grad_desc(X_train, y_train)
    assert len(theta) == len(coefs)
    print(theta)
    for i in range(len(theta)) :
        assert abs(theta[i] - coefs[i]) < .5
...
def test_synthetic1_by_score() :
    ...
    assert pred_and_r2(reg, X_train, y_train) == approx(1., .01)
...
def test_diabetes() :
    ...
    reg = theta_to_function(ridge_regression_grad_desc(diabetes.data[:400], diabetes.target[:400], alpha=.1))
    assert pred_and_r2(reg, diabetes.data[:400], diabetes.target[:400]) == approx(.4, .05)
    assert pred_and_r2(reg, diabetes.data[400:], diabetes.target[400:]) == approx(.47, .05)
...
def test_mpg() :
    ...
    reg = theta_to_function(ridge_regression_grad_desc(X_train, y_train))
    assert pred_and_r2(reg, X_train, y_train) == approx(.81, .02)
    assert pred_and_r2(reg, X_test, y_test) == approx(.5, .1)
